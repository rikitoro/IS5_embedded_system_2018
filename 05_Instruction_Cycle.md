...
表中の reg <- data という表記は右辺のデータ data を左辺のレジスタ reg へ転送することを表します。
例えば MAR <- PC はプログラムカウンタPCに格納されたデータを MAR に転送することを表し、 R <- PC + 1 は(ALUにおいて) PC に1を加えた結果をRレジスタへ転送することを表します。
...
メモリでは読出し操作を行います。表中 RD <- MEM[MAR] は MAR で指定されたアドレスに格納されたデータを RD へ出力することを意味しています。
...
|---|---|---|---|
| P20 | T <- reg | | |
| P21 | A <- A   | | R <- A + T, FLG <- Flag(A + T) |
| P22 | A <- R   | | |

P20 では被加算数となる reg のデータを T レジスタへ転送します。

P21では加算数となる A レジスタのデータを Xbus へ出力するため、A レジスタから自分自身(Aレジスタへの)データ転送を行います。
ALU においては Xbus に出力されたAレジスタの値と、T レジスタの値を加算し、その演算結果をRレジスタへ転送します。
同時に、演算結果によって生じたフラグ信号(S, Z, Cy)をフラグレジスタFLGへ格納します。表中の FLG <- Flag(A + T) は A + T の演算によって生じたフラグ信号を FLG へ転送することを表します。
...
|ステップ| Xbus | Memory | ALU |
|---|---|---|---|
| P10 | reg <- reg   | | R <- reg + 1, FLG <- Flag(reg + 1) |
| P11 | reg <- R     | | |
...
メモリでは、データの読出し(RD <- MEM[MAR])を行います。
...
今後は PC <- PC のような意味のないデータ転送については記述を省略します。
...
ST2ではデータの書き込み先のアドレス指定を行います(MAR <- RD)。

ST3ST3ではメモリへ書き込むデータ( sreg )を WDR へ格納します。

ST2とST3で書き込み先アドレス(MAR)と、書き込みデータ(WDR)が確定したので、次のステップ ST4 ではメモリへのデータの書き込を行います。表中 MEM[MAR] <- WDR はMARで指定されるアドレスを書き込み先として、WDRの内容をメモリへ書き込むことを表します。
以上で、ST命令は終了します。
...
この命令は、サインフラグSが立っている場合(S == 1)はアドレス adrs にある命令へ分岐し、そうでない場合(S==0)の場合は次の命令へ移ります。
...
サインフラグが立っている場合(S == 1)は、 PC を取得した adrs の値で更新します(PC <- RD)。
サインフラグが立っていないの場合(S == 0)は、特に何もせず PC の値は更新しません。
以上でJS命令が終了します。
